...
def is_testbed_prefix(prefix):
	...
	if(re.search("(ndn:/ndn/ucla.edu/%C1\.S\.*)", prefix) != None):
		return False
	if(re.search("(\.edu)|(\.org)|(\.com)|(\.cn)|(\.es)|(\.ch)|(\.fr)|(\.jp)|(\.uk)", prefix) != None):
...
def get_domain(prefix):
	...
	if(re.search("ndn:/.*((edu)|(org)|(com)|(cn)|(es)|(ch)|(jp)|(fr)|(uk))", prefix) == None):
                #print "get_domain(" + prefix + ") returning false: "
		return "" #eval to false
	...
	prime = re.search("ndn:/.*((\edu)|(\org)|(\com)|(\cn)|(es)|(es)|(ch)|(jp)|(fr)|(\uk)).*", prefix).group(0)[5:]
...
def get_all_xml(source_url, output_folder, master_xml_list, wait):
	...
	for node_tuple in source_url:
		...
		p = subprocess.Popen(["wget","-t 2","-T 5","-q","-b","-O",output_folder+temp_filename,node_tuple[0]], stdout=subprocess.PIPE)
                print "wget -t 2 -T 5 -q -b -O " + output_folder + temp_filename + " " + node_tuple[0] + " "
...
<font size="3" face="arial">Notes on current (October 17, 2014) status: Waseda University has joined the NDN Testbed. Their node, WASEDA, is up and running.<br></font>
<font size="3" face="arial">Notes on current (October 29, 2014) status: We are installing an update to nfd and associated utilities. While this is in progress ndnmap.arl.wustl.edu may have periods of idleness..<br></font>
<br>
...
def fes_html_gen(all_prefix):
	...
	html_code += "<br />"
	html_code += "<br />"
	html_code += "<img border=\"0\" src=\"topology.png\" alt = \"testbed\" width=\"1200\" height=\"800\"/></div>\n"
	html_code += "</body>\n</html>\n"
	return html_code
